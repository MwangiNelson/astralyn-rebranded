...
function processImage(img) {
  // This texture only drives the shape/depth mask. Keeping it modest prevents
  // a large synchronous processing spike during the hero's first render.
  const MAX_SIZE = 640;
  const MIN_SIZE = 480;
  ...
  if (!ctx) throw new Error('Could not create a 2D canvas for the logo mask.');
...
export default function MetallicPaint({
  ...
  const resizeCanvas = useCallback(() => {
    ...
    // A full 1000px canvas multiplied by a high-DPR display is expensive to
    // animate. Match the visible logo, but cap the physical backing store.
    const requestedScale = Math.min(window.devicePixelRatio || 1, 1.5);
    const scale = Math.min(requestedScale, 1024 / Math.max(rect.width, rect.height));
  ...
  useEffect(() => {
    if (!initGL()) {
      onErrorRef.current?.(new Error('WebGL 2 is not available.'));
      return;
    }
    ...
    const handleContextLost = (event) => {
      ...
      onErrorRef.current?.(new Error('The metallic logo renderer lost its WebGL context.'));
    };
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      try {
        ...
        // Draw a masked frame before revealing the canvas. This is the guard
        // against the unmasked metallic square flashing during load.
        const gl = glRef.current;
        const uniforms = uniformsRef.current;
        gl.useProgram(programRef.current);
    ...
    img.onerror = () => {
      if (!cancelled) onErrorRef.current?.(new Error(`Could not load metallic logo mask: ${imageSrc}`));
    };
    img.src = imageSrc;
  ...
  useEffect(() => {
    ...
    const draw = time => {
      ...
      if (mouseAnimRef.current && !prefersReducedMotion) {
        mouse.x += (mouse.targetX - mouse.x) * 0.08;
        mouse.y += (mouse.targetY - mouse.y) * 0.08;
        animTimeRef.current = mouse.x * 3000 + mouse.y * 1500;
      } else {
        animTimeRef.current += delta * (prefersReducedMotion ? 0 : speedRef.current);
      }
    ...
    const start = () => {
      if (!active || document.hidden || rafRef.current) return;
  ...
  // Keep the canvas invisible until the shape mask is uploaded and the first
  // masked frame is drawn — otherwise the raw square shader output flashes
  // ("chrome square") before the logo mask takes over.
  return (
    <canvas
      ref={canvasRef}
      className={`paint-container${textureReady ? ' is-ready' : ''}`}
    />
  );
}
